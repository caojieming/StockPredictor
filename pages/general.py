...
layout = dbc.Container([
    ...
    html.Div('Input values into all the fields below, click the "GO" button, and wait for the graph to display. The higher the forecast range chosen, the longer it will take (up to a 3-4 minutes). You can see if the forecast is still running by looking at the title of the tab (should say "Updating...").', style={'margin-top':'20px'}),
    html.Div('Ticker Symbol is the symbol of the stock you\'re trying to forecast.', style={'margin-top':'20px'}),
    html.Div('Price History is how far back in time (relative from today) that you want the graph to show.', style={'margin-top':'20px'}),
    html.Div('Lookback Range is how far back in time (relative from today) the forecaster looks to train its ML model. The period selected will be used to estimate the values within the forecast range. VALUE MUST BE LESS THAN "Price History"!', style={'margin-top':'20px'}),
    html.Div('Forecast Range is how far into the future the model will forecast values. TRY TO KEEP THIS VALUE LESS THAN "Lookback History". Keep in mind that this forecast is not perfect, and the longer the range, the less accurate the model will be.', style={'margin-top':'20px'}),
    ...
    dbc.Row([
        ...
        html.Button('GO', id='go-button', n_clicks=0, style={'width': '100px', 'height':'50px', 'margin-right':'10px', 'margin-top':'20px'},),
...
def plot_data(go_button, ticker, period_hist, lookback_range, forecast_range):
    ...
    period = ''
    match period_hist:
        case '1 Month': period = '1mo'
        case '6 Months': period = '6mo'
    ...
    match lookback_range:
        case '5 Days': lb_range = 5
        case '1 Month': lb_range = 30
        case '6 Months': lb_range = 180
        case '1 Year': lb_range = 365
        case '5 Years': lb_range = 1825

    # translating forecast range
    fc_range = 0
    match forecast_range:
        case '1 Day': fc_range = 1
        case '5 Days': fc_range = 5
        case '1 Month': fc_range = 30
        case '6 Months': fc_range = 180
        case '1 Year': fc_range = 365
